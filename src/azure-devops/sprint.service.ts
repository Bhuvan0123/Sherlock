...
import { PLANNING_WORK_ITEM_FIELDS } from './field-profiles.js';
...
export class SprintService {
    ...
    async getSprintWorkItems(sprint: Sprint, options: { limit?: number } = {}): Promise<WorkItem[]> {
        const ids = await this.workItems.queryIds([wiql.under(FIELD.iterationPath, sprint.path)], {
            includeCompleted: true,
            limit: options.limit ?? 200
        });
        return await this.workItems.getByIds(ids, { profile: PLANNING_WORK_ITEM_FIELDS });
    }

    async getSprintProgress(sprint: Sprint, options: { includeCarryOver?: boolean } = {}): Promise<SprintProgress> {
        const items = await this.getSprintWorkItems(sprint, { limit: 200 });
        const blockedIds = new Set(await this.workItems.blockedSignalIds({ limit: 200 }).catch(() => []));
