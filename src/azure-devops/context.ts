...
import { MINIMAL_WORK_ITEM_FIELDS, PLANNING_WORK_ITEM_FIELDS, HIERARCHY_WORK_ITEM_FIELDS } from './field-profiles.js';
...
export class ProjectContextService {
    ...
    async getWorkItemFieldProjection(profile: string[] = DEFAULT_WORK_ITEM_FIELDS): Promise<string[]> {
        const available = await this.getAvailableFields();
        const projection = profile.filter(field => available.has(field));

        // Planned/actual date aliases used by compact DTOs — keep these small.
        const fieldMapping = new FieldMappingService(this.defaults.project);
        const map = await fieldMapping.getCanonicalMap();
        for (const f of [...map.plannedStart, ...map.plannedEnd, ...map.actualStart, ...map.actualEnd]) {
    ...
    async getCompletedStateNames(): Promise<string[]> {
        return await this.getStateNamesByCategory(['Completed', 'Resolved']);
    }

    async getInProgressStateNames(): Promise<string[]> {
        return await this.getStateNamesByCategory(['InProgress']);
    }

    async getProposedStateNames(): Promise<string[]> {
        return await this.getStateNamesByCategory(['Proposed']);
    }

    async getStateNamesByCategory(wanted: StateCategory[]): Promise<string[]> {
        const categories = await this.getStateCategories();
        const allow = new Set(wanted);
        const names = new Set<string>();
        for (const states of categories.values()) {
            for (const state of states.values()) {
                if (allow.has(state.category)) names.add(state.name);
            }
        }
        return [...names];
    }
